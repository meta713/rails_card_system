...
class Rcs620s:
	...
	def initDevice(self):
		...
		#正規表現で比較(re.IGNORECASEの引数により大文字と小文字を区別しない)
		p = re.compile("usb",re.IGNORECASE)

		for portName in list(serial.tools.list_ports.comports()):
			if p.search(portName[0]) != None:
				try:
					print(portName[0])
                                        self.__ser = serial.Serial(port=portName[0], baudrate=115200, timeout=self.__timeout/1000.0)
		...
		return "can't open all serial port"
	...
	def readWithoutEncryption(self, serviceCode, blockNumber, length):
		...
		for n in range(0, length):
			buf += "\x80"
			buf += chr(blockNumber + n)

		buf = self.cardCommand(buf)

		if (buf is None) :
			return None
		if (len(buf) != (12+16*length)) :
			return None
	...
	def readBlock(self, serviceCode, blockNumber, length):
		...
		if(self.requestService(serviceCode)) :
			return self.readWithoutEncryption(serviceCode, 0, length)
		else:
			return None
